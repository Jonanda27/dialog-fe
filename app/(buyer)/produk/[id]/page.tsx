...
import { 
    ...
    ShoppingCart, ChevronRight, ShieldCheck, ArrowUpRight, ArrowRight
} from "lucide-react";
            ...
            <main className="max-w-7xl mx-auto px-6 lg:px-10">
                ...
                <div className="flex flex-col gap-6 mb-10">
                    <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-zinc-500">
                        ...
                        <Link href="/search" className="hover:text-[#ef3333] transition-colors">Marketplace</Link>
                        <ChevronRight size={12} />
                        <span className="text-zinc-200 truncate">{product.name}</span>
                    </div>
                    <button onClick={() => router.back()} className="flex items-center gap-2 text-zinc-400 hover:text-white text-[10px] font-black uppercase tracking-[0.2em] group">
                    ...
                    <div className="lg:col-span-6 space-y-8">
                        <div>
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div className="flex items-center gap-1 text-yellow-500 font-black text-xs">
                                    <Star size={14} fill="currentColor" /> {averageRating} <span className="text-zinc-500 ml-1">({totalReviews} Reviews)</span>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {[
                                ...
                                <div key={i} className="bg-zinc-900/50 border border-zinc-800/60 p-4 rounded-2xl">
                        ...
                        <div className="bg-amber-500/5 border border-amber-500/20 rounded-3xl p-6 flex items-start gap-4 shadow-inner">
                            ...
                            <div className="flex-1">
                                <h4 className="text-sm font-black text-amber-500 uppercase tracking-widest mb-1">PRO-GRADING SERVICE</h4>
                                <p className="text-[11px] text-amber-200/60 leading-relaxed font-medium">
                                    Ragu dengan grading penjual? Request pengecekan fisik oleh tim Analog.id Expert (+Rp25.000).
                                </p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mt-20">
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        <ReviewSidebar reviews={reviews} isLoading={isLoadingReviews} total={reviews.length} />
                    </div>
                </div>
                ...
                <div className="mt-32 space-y-10">
                    <div className="flex items-center justify-between border-b border-zinc-900 pb-8">
                        <h2 className="text-3xl font-black text-white uppercase tracking-tighter flex items-center gap-4">
                            <span className="w-2.5 h-10 bg-[#ef3333] rounded-full" /> Market Highlights
                        </h2>
                        <Link href="/search" className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] hover:text-white transition-all flex items-center gap-2">
                            Explore All <ArrowRight size={16} />
                        </Link>
                    </div>
                    <Suspense fallback={<div className="h-96 w-full bg-zinc-900 animate-pulse rounded-[3rem]" />}>
                        <RecommendedFeed />
                    </Suspense>
                </div>
            </main>
