      ...
      <div className="max-w-7xl mx-auto pb-24 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8 md:mb-10">
          <div className="text-center md:text-left">
            <h2 className="text-xl sm:text-2xl font-black uppercase tracking-tight text-white flex items-center justify-center md:justify-start gap-3">
              <Receipt className="text-[#ef3333]" size={28} />
              Semua Transaksi
            </h2>
            <p className="text-xs sm:text-sm text-zinc-500 font-medium mt-2 max-w-xl mx-auto md:mx-0">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-[#1a1a1e] border border-zinc-800 hover:bg-zinc-800 text-zinc-400 hover:text-white px-5 sm:px-6 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">
                <Filter size={16} /> Filter
              </button>
              <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-[#ef3333] hover:bg-red-700 text-white shadow-lg shadow-red-900/20 px-5 sm:px-6 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">
        ...
        {/* METRIK MINI - Horizontal Scroll di Mobile, Grid di Tablet/Desktop */}
        <div className="flex md:grid overflow-x-auto md:overflow-visible snap-x snap-mandatory md:snap-none grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 pb-4 md:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          <div className="w-[85vw] sm:w-[45vw] md:w-full snap-center shrink-0 bg-[#111114] border border-zinc-900 rounded-[2rem] p-5 sm:p-6">
          ...
          <div className="w-[85vw] sm:w-[45vw] md:w-full snap-center shrink-0 bg-[#111114] border border-zinc-900 rounded-[2rem] p-5 sm:p-6">
            ...
            <p className="text-xl sm:text-2xl font-black text-amber-500 tracking-tight truncate">{adminOrders.length}</p>
          </div>
          <div className="w-[85vw] sm:w-[45vw] md:w-full snap-center shrink-0 bg-[#111114] border border-zinc-900 rounded-[2rem] p-5 sm:p-6">
            <p className="text-[9px] font-black text-zinc-600 uppercase tracking-widest mb-1">Status Sistem</p>
            <p className="text-xl sm:text-2xl font-black text-blue-500 tracking-tight uppercase text-xs truncate">Synchronized</p>
          </div>
          <div className="w-[85vw] sm:w-[45vw] md:w-full snap-center shrink-0 bg-[#111114] border border-zinc-900 rounded-[2rem] p-5 sm:p-6 border-l-2 md:border-l-0 lg:border-l-2 border-l-purple-500 md:border-t-2 md:border-t-purple-500 lg:border-t-0">
        ...
        {/* TABLE CONTENT */}
        <div className="bg-[#111114] border border-zinc-900 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl">
          <div className="overflow-x-auto custom-scrollbar">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-20 text-zinc-500 min-w-[800px]">
              ...
              <div className="p-20 text-center text-red-500 uppercase text-xs font-black tracking-widest min-w-[800px]">{error}</div>
            ) : (
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-[#1a1a1e]/50 border-b border-zinc-900 text-[10px] uppercase tracking-[0.2em] text-zinc-600 font-black">
                    <th className="py-5 sm:py-6 px-6 sm:px-8 whitespace-nowrap">ID Transaksi</th>
                    <th className="py-5 sm:py-6 px-4 whitespace-nowrap">Pembeli & Penjual</th>
                    <th className="py-5 sm:py-6 px-4 whitespace-nowrap">Total</th>
                    <th className="py-5 sm:py-6 px-4 whitespace-nowrap">Status Transaksi</th>
                    <th className="py-5 sm:py-6 px-6 sm:px-8 text-right whitespace-nowrap">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-900">
                  {filteredTransactions.map((trx) => (
                    <tr key={trx.id} className="hover:bg-white/[0.01] transition-colors group">
                      ...
                      <td className="py-4 sm:py-5 px-4">
                        <div className="space-y-1.5">
                          <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                            <User size={12} className="text-blue-500 shrink-0" /> <span className="truncate max-w-[120px] sm:max-w-[150px]">{trx.buyer?.full_name || 'Buyer'}</span>
                          </div>
                          <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                            <Store size={12} className="text-emerald-500 shrink-0" /> <span className="truncate max-w-[120px] sm:max-w-[150px]">{trx.store?.name || 'Store'}</span>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 sm:py-5 px-4">
                      ...
                      <td className="py-4 sm:py-5 px-4">
                        <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border inline-block whitespace-nowrap ${getStatusColor(trx.status)}`}>
                      ...
                      <td className="py-4 sm:py-5 px-6 sm:px-8">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg bg-[#1a1a1e] text-zinc-400 hover:text-white border border-zinc-800 hover:border-zinc-700 transition-all text-[9px] font-black uppercase tracking-widest"
          ...
          <div className="p-4 sm:p-6 border-t border-zinc-900 bg-[#0d0d0f] flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-[9px] font-bold text-zinc-600 uppercase tracking-[0.2em] text-center sm:text-left">Showing {filteredTransactions.length} platform-wide transactions</p>
            <div className="flex gap-2 w-full sm:w-auto">
              <button className="flex-1 sm:flex-none px-4 py-2 rounded-lg bg-[#1a1a1e] text-zinc-600 text-[9px] font-black uppercase tracking-widest border border-zinc-900 cursor-not-allowed">Prev</button>
              <button className="flex-1 sm:flex-none px-4 py-2 rounded-lg bg-[#1a1a1e] text-zinc-400 hover:text-white text-[9px] font-black uppercase tracking-widest border border-zinc-900 transition-all">Next</button>
            </div>
          </div>
        </div>
        ...
        {selectedTrx && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="relative w-full max-w-[800px] max-h-[90vh] flex flex-col bg-[#111114] border border-zinc-800 rounded-[2rem] sm:rounded-[3rem] shadow-2xl animate-in zoom-in-95 duration-300 overflow-hidden">

              {/* Modal Header */}
              <div className="bg-[#1a1a1e] px-6 sm:px-8 py-5 sm:py-6 border-b border-zinc-900 flex justify-between items-start sm:items-center shrink-0">
                <div className="pr-4">
                  <h3 className="text-sm font-black text-white uppercase tracking-wider flex items-center gap-2">
                    <Receipt size={18} className="text-[#ef3333] shrink-0" /> <span className="line-clamp-1">Detail Transaksi Platform</span>
                  </h3>
                  <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-1">ID: {selectedTrx.id}</p>
                </div>
                <button onClick={() => setSelectedTrx(null)} className="text-zinc-500 hover:text-white transition-colors bg-zinc-900 p-2 rounded-full shrink-0">
                  <X size={18} />
                </button>
              </div>

              {/* Modal Body */}
              <div className="p-6 sm:p-8 overflow-y-auto custom-scrollbar text-left flex-1">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-6 sm:mb-8">
                  ...
                  <div className="flex-1 p-5 rounded-2xl bg-[#0a0a0b] border border-zinc-800">
                    ...
                    <p className={`text-sm sm:text-base font-black uppercase tracking-tight ${getEscrowColor(selectedTrx.status)}`}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-zinc-500 uppercase mt-0.5 break-all">{selectedTrx.buyer?.email}</p>
                ...
                <div className="bg-[#0a0a0b] border border-zinc-900 rounded-2xl p-4 sm:p-5 space-y-4">
                  {selectedTrx.items?.map((item, idx) => (
                    <div key={idx} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 pb-3 border-b border-zinc-900 last:border-0 last:pb-0">
                      <div className="flex items-start sm:items-center gap-3">
                        <Package size={16} className="text-zinc-500 shrink-0 mt-0.5 sm:mt-0" />
                        <span className="text-xs font-bold text-zinc-300 uppercase leading-snug">{item.product?.name} ({item.qty}x)</span>
                      </div>
                      <span className="text-xs font-black text-white sm:text-right pl-7 sm:pl-0">{toIDR(item.price_at_purchase)}</span>
                    </div>
                  ))}
                  ...
                  <div className="border-t border-zinc-800 pt-4 flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-2">
                    ...
                    <span className="text-lg sm:text-xl font-black text-[#ef3333]">
                ...
                {selectedTrx.status === "disputed" && (
                  <div className="mt-6 sm:mt-8 bg-purple-500/10 border border-purple-500/20 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-start sm:items-center gap-3">
                      <AlertOctagon className="text-purple-500 shrink-0 mt-1 sm:mt-0" size={24} />
                      <div>
                        <p className="text-xs font-black text-white uppercase tracking-tight">Transaksi dalam Dispute</p>
                        <p className="text-[10px] text-zinc-400 font-medium mt-1 sm:mt-0">Investigasi diperlukan sebelum pelepasan dana.</p>
                      </div>
                    </div>
                    <button className="w-full sm:w-auto bg-purple-600 hover:bg-purple-700 text-white text-[9px] font-black uppercase tracking-widest px-4 py-3 sm:py-2.5 rounded-xl transition-colors">
                      Pusat Resolusi
                    </button>
                  </div>
                )}
