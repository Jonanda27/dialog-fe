...
import { 
    Search, ShoppingBag, LogOut, Heart, Bell, ChevronDown, 
    Zap, HelpCircle, Globe, Wallet, Plus, Loader2, LayoutGrid,
    ArrowRight, Star, Trash2
} from 'lucide-react';
...
// Perbaikan fungsi getImageUrl lokal jika helper global bermasalah
const formatImageUrl = (path: string | undefined) => {
    if (!path) return 'https://placehold.co/400x400?text=No+Image';
    if (path.startsWith('http')) return path;
    // Mengarahkan ke localhost:5000 sesuai permintaan Anda
            ...
            if (isInitialized && user?.role === 'seller') {
                try {
                    ...
                    if (res.data && res.data.balance) {
                        const numericBalance = typeof res.data.balance === 'string' ? parseFloat(res.data.balance) : res.data.balance;
                        setWalletBalance(numericBalance || 0);
                    }
                } catch (error) { console.error(error); } finally { setIsLoadingWallet(false); }
            }
        };
        fetchWallet();
    }, [user, isInitialized]);
        ...
        <header className="fixed top-0 w-full z-[100] transition-all duration-500">
            ...
            <div className="bg-[#0a0a0b] border-b border-zinc-900/50 py-2.5 px-6 hidden lg:block">
                <div className="w-full flex justify-between items-center text-[10px] font-black uppercase tracking-[0.15em]">
                    <div className="flex items-center gap-8">
                        ...
                        <Link href="/sell" className="text-[#ef3333] hover:text-red-400 transition-colors flex items-center gap-1.5"><Zap size={12} fill="currentColor" /> Sell on Analog</Link>
                        <Link href="/help" className="text-zinc-500 hover:text-white transition-colors flex items-center gap-1.5"><HelpCircle size={12} /> Help</Link>
                    </div>
            ...
            <div className={`transition-all duration-500 px-6 ${scrolled ? "bg-[#111114]/95 backdrop-blur-md py-4 shadow-2xl" : "bg-[#0a0a0b] py-6"}`}>
                <div className="w-full flex items-center justify-between gap-12">
                    <Link href="/" className="text-3xl font-black text-[#ef3333] tracking-tighter uppercase shrink-0">Analog<span className="text-white">.id</span></Link>
                    ...
                    <div className="flex items-center gap-8 shrink-0">
                        <button className="relative text-zinc-400 hover:text-white transition-colors hidden sm:block"><Bell size={24} /><span className="absolute top-0 right-0 w-2.5 h-2.5 bg-[#ef3333] rounded-full border-2 border-[#0a0a0b]"></span></button>
                            ...
                            <div className="flex items-center gap-6">
                                <Link href="/dashboard" className="flex items-center gap-4 group">
                                    <div className="relative">
                                        <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user.full_name}&backgroundColor=ef3333`} className="w-11 h-11 rounded-full border-2 border-zinc-800 group-hover:border-[#ef3333] transition-all" alt="Avatar" />
                                        <div className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-emerald-500 border-2 border-[#111114] rounded-full"></div>
                                    </div>
                                    <div className="hidden xl:flex flex-col text-left leading-none"><span className="text-[11px] font-black text-white uppercase tracking-tight">{user.full_name.split(' ')[0]}</span><span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Collector</span></div>
                                </Link>
                                <button onClick={() => logout()} className="text-zinc-600 hover:text-[#ef3333] transition-all"><LogOut size={20} /></button>
                            </div>
                        ) : (
                            <div className="flex items-center gap-8"><Link href="/auth/login" className="text-[11px] font-black uppercase tracking-widest text-zinc-400 hover:text-white">Sign In</Link><Link href="/auth/register" className="bg-white text-black px-10 py-3.5 rounded-full text-[11px] font-black uppercase tracking-widest hover:bg-[#ef3333] hover:text-white transition-all shadow-xl">Join</Link></div>
                        )}
                    </div>
                </div>
            </div>
                            ...
                            <div className="absolute top-full left-1/2 -translate-x-1/2 w-screen max-w-[1000px] bg-[#111114] border border-zinc-800 shadow-[0_30px_60px_rgba(0,0,0,0.8)] p-8 rounded-b-[3rem] opacity-0 translate-y-4 pointer-events-none group-hover/all:opacity-100 group-hover/all:translate-y-0 group-hover/all:pointer-events-auto transition-all duration-300 z-[110]">
                                <div className="grid grid-cols-12 gap-10 text-left">
                                            ...
                                            <div key={cat.id} className="space-y-4">
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    {cat.subCategories?.map((sub: any) => (
                                                        <Link key={sub.id} href={`/search?sub_category=${sub.id}`} className="text-[10px] font-bold text-zinc-500 hover:text-[#ef3333] transition-colors">{sub.name}</Link>
                                                    ))}
                                                </div>
                                            </div>
                                        ))}
                                    ...
                                    <div className="col-span-4 bg-zinc-900/50 rounded-[2rem] p-6 border border-zinc-800 flex flex-col justify-between">
                                        <div>
                                            <div className="w-10 h-10 bg-[#ef3333]/10 rounded-full flex items-center justify-center text-[#ef3333] mb-4"><Star size={20} fill="currentColor" /></div>
                                        ...
                                        <div className="max-w-7xl mx-auto grid grid-cols-12 gap-16 text-left">
                                            ...
                                            <div className="col-span-3">
                                                ...
                                                <div className="flex flex-col gap-4">
                                                    ...
                                                    <Link href="/deals" className="text-sm font-black text-[#ef3333] flex items-center gap-2 mt-4 underline underline-offset-8">Explore All {cat.name} <ArrowRight size={14}/></Link>
                                                </div>
                                            </div>
